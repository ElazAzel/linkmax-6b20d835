...
export const emailSequencesService = {
  ...
  async getSequenceDetails(id: string) {
    try {
      const { data: sequence, error: seqError } = await (supabase as unknown as { from: (schema: string) => any })
        .from('email_sequences')
        .select('*')
        .eq('id', id)
        .single();
      ...
      const { data: steps, error: stepsError } = await (supabase as unknown as { from: (schema: string) => any })
