...
export default function PublicPageClient({ initialPageData }: Props) {
    const { t } = useTranslation();
    const params = useParams();
    const slug = params?.slug as string | undefined;
    const compressed = params?.compressed as string | undefined;
    ...
    // Use window.location only after mount to avoid hydration mismatch, or use a hook
    ...
    // Use React Query for caching, initialized with server data
    const { data: pageData, isLoading } = useQuery({
        queryKey: ['public-page', slug || compressed],
        queryFn: async () => initialPageData, // In client, we relying on server data or initial props
        initialData: initialPageData,
        staleTime: 60 * 1000,
    });

    const loading = isLoading && !pageData;
    ...
    return (
        <>
            ...
            {/* Note: Server side metadata handles primary tags, these might be redundant but kept for dynamic updates */}
            <EnhancedSEOHead
                pageData={pageData}
            ...
            {/* AISearchOptimizer is client-side, enabling dynamic LD-JSON injection for AI Search Readiness */}
            <AISearchOptimizer
                pageType="profile"
            ...
            {/* Crawler-friendly content for no-JS fallback (Hidden but present in DOM) */}
