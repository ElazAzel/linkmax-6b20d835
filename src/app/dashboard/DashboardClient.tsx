 ...
 * Migrated from DashboardV2.tsx to use Next.js App Router
 */
...
export default function DashboardClient() {
    ...
    // Helper for setting search params
    ...
    const { canUseCustomPageBackground } = useFreemiumLimits(); // Imported but unused in original?
    ...
    const currentTab = useMemo((): TabId => {
        ...
        const lastPart = pathParts[pathParts.length - 1]; // e.g. 'home' or 'events'
        // If we are at /dashboard, default to home
    ...
    const handleTabChange = useCallback((tabId: string) => {
        if (tabId === 'home') {
            ...
            router.push('/dashboard/home?tab=editor');
        } else {
                    ...
                    <Suspense fallback={<ScreenLoader />}>
                        ...
                        {currentTab === 'pages' && (
                            <PagesScreen
                                ...
                                onUpgradePlan={() => router.push('/pricing')}
                            />
                        )}

                        {currentTab === 'activity' && (
                            <ActivityScreen isPremium={dashboard.isPremium} />
                        )}
                        ...
                        {currentTab === 'insights' && (
                            <InsightsScreen
                                ...
                                isPremium={dashboard.isPremium}
                                onApplyInsight={(action) => handleTabChange('home')}
                            />
                        )}
                        ...
                        {currentTab === 'settings' && (
                            <SettingsScreen
                                ...
                                seoTitle={dashboard.pageData?.seo?.title || ''}
                                seoDescription={dashboard.pageData?.seo?.description || ''}
                        ...
                        {currentTab === 'events' && (
                            (pathname.match(/\/dashboard\/events\/[^/]+$/) && !pathname.includes('/scanner')) ? (
                                <EventDetailScreen />
                            ) : (
                                <EventsScreen />
                            )
