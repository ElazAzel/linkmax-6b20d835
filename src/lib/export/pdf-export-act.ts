/**
 * PDF Export utility for Zone Acts (Акт выполненных работ)
 * Based on pdf-export-invoice.ts pattern with jsPDF + autoTable
 * Supports Cyrillic via transliteration
 */
import { format } from 'date-fns';
import type { ZoneInvoice, ZoneInvoiceItem } from '@/types/zones';

export interface ActPDFOptions {
    invoice: ZoneInvoice;
...
export async function exportActToPDF({
    invoice,
    items,
    zoneName,
}: ActPDFOptions): Promise<void> {
    ...
    // Akt vypolnennykh rabot (Act of performed work)
    doc.text(safe(`Akt vypolnennykh rabot k Schetu #${invoice.invoice_number || ''}`), 14, y);
    y += 10;
    ...
    doc.text(safe(`Ispolnitel: ${zoneName}`), 14, y);
    y += 5;

    if (invoice.contact) {
        doc.text(safe(`Zakazchik: ${invoice.contact.name || ''}`), 14, y);
        y += 5;
    }
    y += 5;
    ...
    // ─── Introductory Text ───
    doc.setFontSize(9);
    doc.text(safe('My, nizhepodpisavshiesya, sostavili nastoyashchiy akt o tom, chto ispolnitel vypolnil, a zakazchik prinyal sleduyushchie raboty (uslugi):'), 14, y, { maxWidth: pageWidth - 28 });
    y += 10;

    // ─── Items table ───
    const tableHeaders = ['#', safe('Naimenovanie rabot, uslug'), safe('Kol-vo'), safe('Tsena'), safe('Summa')];
    ...
    autoTable(doc, {
        ...
        theme: 'grid',
        headStyles: {
            fillColor: [71, 85, 105], // Slate color for Act
            textColor: 255,
            fontSize: 9,
            fontStyle: 'bold',
        },
    ...
    doc.setTextColor(40, 40, 40);
    doc.text(
        safe(`Itogo k oplate: ${total.toLocaleString()} ${invoice.currency || 'KZT'}`),
        pageWidth - 14,
        y,
        { align: 'right' },
    );
    ...
    doc.setFontSize(9);
    doc.text(safe('Vysheperechislennye uslugi vypolneny polnostyu i v srok. Zakazchik pretenziy po obemu, kachestvu i srokam okazaniya uslug ne imeet.'), 14, y, { maxWidth: pageWidth - 28 });
    y += 15;

    // ─── Signatures ───
    doc.setFontSize(10);
    doc.text(safe('Ot Ispolnitelya:'), 14, y);
    doc.line(40, y + 1, 90, y + 1); // Signature line
    
    doc.text(safe('Ot Zakazchika:'), pageWidth / 2 + 14, y);
    doc.line(pageWidth / 2 + 40, y + 1, pageWidth - 14, y + 1); // Signature line
    ...
    doc.save(`act_${number}_${format(new Date(), 'yyyy-MM-dd')}.pdf`);
}
