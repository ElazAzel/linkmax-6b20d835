...
export default function AuthCallback() {
    ...
    useEffect(() => {
        ...
        if (oauthParams.error) {
            ...
            supabase.auth.getSession().then(({ data: { session } }) => {
                ...
                if (session) {
                    ...
                    navigate(`/dashboard/settings?auth_error=${encode(oauthParams.error || '')}&auth_error_description=${encode(oauthParams.errorDescription || '')}`, { replace: true });
                } else {
                    // Regular login error
                    navigate(`/auth?auth_error=${encode(oauthParams.error || '')}&auth_error_description=${encode(oauthParams.errorDescription || '')}`, { replace: true });
                }
            });
            return;
        }
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange((event) => {
            if (event === "SIGNED_IN") {
                navigate(oauthParams.returnTo, { replace: true });
            }
        });
