...
export const COMPANY_DETAILS = {
  name: 'ARchitecKZ',
  address: 'г. Алматы, ул. Шолохова, д. 20',
  bin: '190540008684',
  bank: 'АО "Kaspi Bank"',
  kbe: '19',
  bik: 'CASPKZKA',
  ...
  const getTermsContent = () => {
    if (lang === 'en') {
      return (
        <>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`1.2. Platform - the website at ${getAppDomain()}, related subdomains, API, and the progressive web application (PWA) providing access to page creation and management, analytics, CRM, and other services.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`1.4. Privacy Policy - a document defining the procedure for processing and protecting Users' personal data, posted at ${getAppDomain()}/privacy.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`2.2. Use of the Platform is subject to: this Agreement; the Privacy Policy; the terms of tariffs and paid services published at ${getAppDomain()}/pricing.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`3.1. Registration is done through the web interface at ${getAppDomain()} using: email and password and/or; login via third-party services (Google, Apple) and/or; verification via Telegram bot.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`5.3. Prices, tariff composition and payment terms are published on the tariffs page at ${getAppDomain()}/pricing and may be changed by the Administration unilaterally for new periods.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`10.4. The current version of the Agreement is always available at: ${getAppDomain()}/terms.`}</p>
    ...
    if (lang === 'kk') {
      return (
        <>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`1.2. Платформа - ${getAppDomain()} мекенжайындағы веб-сайт, байланысты субдомендер, API, сондай-ақ беттерді жасау мен басқаруға, аналитикаға, CRM-ге және басқа қызметтерге қол жеткізуді қамтамасыз ететін прогрессивті веб-қосымша (PWA).`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`2.2. Платформаны пайдалану: осы Келісім; Құпиялылық саясаты; ${getAppDomain()}/pricing сайтында жарияланған тарифтер мен ақылы қызметтер шарттары негізінде жүзеге асырылады.`}</p>
          
          <h3 className="font-semibold mt-6 mb-2">3. Тіркелу және тіркелгіге қол жеткізу</h3>
          <p className="text-sm text-muted-foreground mb-2">{`3.1. Тіркелу ${getAppDomain()} мекенжайындағы веб-интерфейс арқылы жүзеге асырылады: email және құпия сөз және/немесе; үшінші тарап қызметтері арқылы кіру (Google, Apple) және/немесе; Telegram боты арқылы растау.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`5.3. Ағымдағы бағалар, тарифтік жоспарлар мен төлем шарттары ${getAppDomain()}/pricing тарифтер бетінде жарияланған.`}</p>
          ...
          <p className="text-sm text-muted-foreground mb-2">{`10.4. Келісімнің ағымдағы нұсқасы әрқашан мына мекенжайда қол жетімді: ${getAppDomain()}/terms.`}</p>
    ...
    return (
      <>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`1.2. Платформа - веб-сайт по адресу ${getAppDomain()}, связанные поддомены, API, а также прогрессивное веб-приложение (PWA), предоставляющие доступ к функционалу по созданию и управлению страницами, аналитике, CRM и другим сервисам.`}</p>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`1.4. Политика конфиденциальности - документ, определяющий порядок обработки и защиты персональных данных Пользователей, размещённый по адресу ${getAppDomain()}/privacy.`}</p>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`2.2. Использование Платформы осуществляется на условиях: настоящего Соглашения; Политики конфиденциальности; условий тарифов и платных сервисов, опубликованных на ${getAppDomain()}/pricing.`}</p>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`3.1. Регистрация осуществляется через веб-интерфейс по адресу ${getAppDomain()} с использованием: email и пароля и/или; входа через сторонние сервисы (Google, Apple) и/или; верификации через Telegram-бота.`}</p>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`5.3. Цены, состав тарифов и условия оплаты публикуются на странице тарифов ${getAppDomain()}/pricing и могут изменяться Администрацией в одностороннем порядке для новых периодов.`}</p>
        ...
        <p className="text-sm text-muted-foreground mb-2">{`10.4. Актуальная версия Соглашения всегда доступна по адресу: ${getAppDomain()}/terms.`}</p>
