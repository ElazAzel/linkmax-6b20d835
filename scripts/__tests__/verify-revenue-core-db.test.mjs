...
test('dry run lists every destructive and verification step in safe order', () => {
  ...
  const start = result.stdout.indexOf('supabase start');
  const reset = result.stdout.indexOf('supabase db reset --local');
  const pgTap = result.stdout.indexOf('supabase test db --local supabase/tests');
  const lint = result.stdout.indexOf('supabase db lint --local --schema public');
