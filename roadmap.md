...
## Open (blocked: hosted database paused / needs staging credentials)
...
- [ ] Apply migration `admin_set_user_tier` RPC to live DB (needed for admin tier changes).
- [ ] Update `.env` with staging Supabase `SUPABASE_PROJECT_ID`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY` once user provides them.
