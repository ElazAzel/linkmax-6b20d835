...
-- P2.9: Child search control plane — add child entity metadata to indexing_submissions

ALTER TABLE public.indexing_submissions
  ADD COLUMN IF NOT EXISTS child_item_id text,
  ADD COLUMN IF NOT EXISTS child_slug text;

-- Index for looking up submissions by child entity
CREATE INDEX IF NOT EXISTS idx_indexing_submissions_child 
  ON public.indexing_submissions(page_id, child_item_id) 
  WHERE child_item_id IS NOT NULL;

-- Update diagnostics RPC to include per-child last submission info
...
BEGIN
  ...
  -- Parse id-keyed service_slugs with per-child submission lookup
  ...
  LOOP
    ...
    -- Lookup last submission for this child
  ...
  -- Last 20 indexing submissions (increased to show child entries too)
