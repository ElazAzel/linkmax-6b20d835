...
-- P2.7: Update save_page_blocks to auto-generate IDs for pricing items without one
-- Instead of skipping items with missing/short IDs, generate a stable auto-id server-side.
...
BEGIN
  ...
  LOOP
    IF v_block->>'type' = 'pricing' THEN
      ...
      LOOP
        ...
        -- AUTO-GENERATE ID for items without one (P2.7 safeguard)
  ...
  -- Block count scoring
  IF jsonb_array_length(p_blocks) >= 4 THEN
    v_score := v_score + 10;
  ELSIF jsonb_array_length(p_blocks) >= 2 THEN
    v_score := v_score + 5;
  ELSE
  ...
  v_breakdown := jsonb_build_object(
    ...
    'blocks', jsonb_build_object('passed', jsonb_array_length(p_blocks) >= 2, 'points',
      CASE WHEN jsonb_array_length(p_blocks) >= 4 THEN 10
           WHEN jsonb_array_length(p_blocks) >= 2 THEN 5
           ELSE 0 END)
  );
