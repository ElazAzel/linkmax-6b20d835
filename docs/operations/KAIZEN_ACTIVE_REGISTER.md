...
| KZN-010 | CRM | P0 | Implemented safe step | Ускорить и измерить обработку lead через быстрые действия. | CRM response time | CRM/Operations owner | `LeadsScreen` теперь пишет reply/status events и фильтрует status update по `user_id`. |
