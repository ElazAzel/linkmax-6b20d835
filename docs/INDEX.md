# Documentation Index

Welcome to the **lnkmx** developer & stakeholder portal.

---

## 📚 Core Documentation

- **[Platform Snapshot](./PLATFORM_SNAPSHOT.md)**: **Start here.** Single source of truth for architecture, features, and system status.
- **[Changelog](./CHANGELOG.md)**: Track version history and notable changes.
- **[Features](./Features.md)**: Complete feature inventory with implementation status.

## 🏗️ Architecture & Technical

- **[Architecture (Overview)](./architecture.md)**: System design, data flow, and component diagram.
- **[Architecture Patterns](./ARCHITECTURE_PATTERNS.md)**: *NEW* State management and improved component patterns.
- **[Stack References](./STACK_REFERENCES.md)**: *NEW* Libraries, tools, and best practices.
- **[Performance Guide](./PERFORMANCE_GUIDE.md)**: *NEW* Optimization strategies for Frontend/Backend.
- **[Database Schema](./DATABASE_SCHEMA_GUIDE.md)**: *NEW* ERD and JSON field structures.
- **[Security & Ops](./SECURITY_OPS.md)**: *NEW* Incident response and security protocols.
- **[Accessibility Checklist](./A11Y_CHECKLIST.md)**: *NEW* A11y standards.
- **[API & Backend](./API.md)**: 28 edge functions, RPCs, and RLS documentation.
- **[Security (General)](./SECURITY.md)**: Security checklist, GDPR, rate limiting, and compliance.
- **[UI Components](./UI-COMPONENTS.md)**: Guide to shared UI components.
- **[Dependency Map](./DEPENDENCY_MAP.md)**: Module dependency graph.
- **[Testing](./TESTING.md)**: Testing strategies and tools.

## 📖 Business & Strategy

The **6 Pillars** — core strategic documents:

1. [**Product Vision & Strategy**](./1_PRODUCT_VISION.md) — *the "Why"*
2. [**Platform Architecture**](./2_PLATFORM_ARCHITECTURE.md) — *the "How"*
3. [**Marketing & GTM Strategy**](./3_MARKETING_STRATEGY.md) — *the "Growth"*
4. [**Investment Memo**](./4_INVESTMENT_MEMO.md) — *the "Business Case"*
5. [**Product Roadmap**](./5_PRODUCT_ROADMAP.md) — *the "Timeline"*
6. [**Operational Handbook**](./6_OPERATIONAL_HANDBOOK.md) — *the "Runbook"*

Additional:
- **[Pitch Deck](./PITCH_DECK.md)**: Investor presentation.
- **[Competitive Notes](./COMPETITIVE_NOTES.md)**: Market analysis, competitor teardowns, and trend radar.

## 🛠️ Operations & Runbooks

- **[Deployment Runbook](./RUNBOOKS/DEPLOYMENT.md)**: Deploy to production safely.
- **[Local Dev Runbook](./RUNBOOKS/LOCAL_DEVELOPMENT.md)**: Set up local environment.
- **[Contributing](./../CONTRIBUTING.md)**: Guidelines for contributing code.

## 📝 Architecture Decision Records (ADRs)

- **[ADRs](./ADR/)**: Recorded architectural decisions.

## 🌐 Feature-Specific Docs

- **[Future Stack](./FUTURE_STACK.md)**: *NEW* Gap analysis — tools & services to integrate next.
- **[i18n Operations](./i18n_ops.md)**: Internationalization workflows.
- **[Translation Playbook](./translation_playbook.md)**: Content translation guide.
- **[Demo Accounts](./demo-accounts.md)**: Demo/seed account setup.
- **[Multi-Page](./multi-page.md)**: Multi-page architecture.
- **[Dashboard Redesign](./dashboard-redesign.md)**: Dashboard UX evolution.
- **[Event Block](./event-block.md)**: Event registration block docs.
- **[AEO/GEO Implementation](./aeo-geo-implementation.md)**: Search engine optimization.
- **[SEO docs](./seo/)**: SEO technical documentation.

## 🔍 Audits & Reports

- **[Audits Archive](./audits/)**: Historical audit reports and remediation plans.
- **[Deep Security Audit (2026-02-18)](./DEEP_SECURITY_AUDIT_2026_02_18.md)**: Latest security audit.
- **[Full Platform Audit (2026-02-18)](./FULL_PLATFORM_AUDIT_2026_02_18.md)**: Latest platform audit.
- **[Audit Report (2026-02-27)](./AUDIT_REPORT_2026_02_27.md)**: *Latest* audit of docs and orchestration.
- **[Health Check (2026-02-14)](./health-check-2026-02-14.md)**: System health snapshot.

---

*This file is the authoritative index. `PLATFORM_SNAPSHOT.md` is the technical source of truth.*
