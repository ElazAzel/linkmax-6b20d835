...
- Исправлена критическая ошибка `TypeError: Cannot set properties of undefined (setting 'Children')` путем отката несовместимых версий `react-i18next` and `react-router-dom`.
...
- **Vite Configuration**: Optimization of `build.rollupOptions` for better code splitting and reduced TBT.
...
- **E2E Navigation (v3.5)**: Corrected dashboard tab routing from `/dashboard?tab=crm` to `/dashboard?tab=activity` in E2E tests to match actual component locations.
...
- Replaced hardcoded `lnkmx` → `LinkMAX` in DashboardV2 SEO defaults.

## [2026-03-07] - Monetization Pivot & Global Documentation Sync (v2.5)

### Monetization (Step-by-Growth - 2026-03-07)
...
### Global Documentation Audit (2026-03-07)
...
### Product Strategy (2026-03-07)

- Создан документ `2_BUSINESS_FINANCIAL_MODEL.md` с детальным расчетом юнит-экономики и CAC/LTV.
- Обновлены `STRATEGIC_PLAN_2026.md` and `PRODUCT_ROADMAP.md` (Success-First запуск в Q2).
...
### [2026-02-27] - Documentation Linting & SEO

#### Linter (Feb 27)

- Resolved over 100 Markdown linting warnings across `COMPREHENSIVE_PLATFORM_GUIDE.md`, `DEEP_SECURITY_AUDIT_2026_02_18.md`, and `PLATFORM_SNAPSHOT.md`.
- Unified table styles (compact/aligned pipes), enforced language tagging for code blocks, and corrected header hierarchy for the "Encyclopedia" version of the docs.
- Fixed broken numbering in methodologies and week-to-week priority lists in audit reports.

#### SEO/AI Optimization (Feb 27)

- Completed comprehensive platform audit. Verified full migration to `react-router-dom` and removed legacy `next/link` dependencies.
- Implemented `AISearchOptimizer` for AI-powered search (AEO). Added GEO-tagging for Kazakhstan/Almaty.
- Rewrote `SeoLanding.tsx` into a bilingual Bot Hub for improved indexing.
- Automated dynamic sitemap generation via Edge Functions.
...
## [2026-02-23] - Bugfix: White Screen Remediation
...
- **Landing V2 Translation Alignment**: Translated newly identified strings in the updated `InteractiveDemo` and `BentoGridSection` V2 components completely ensuring 100% cyrillic removal in EN/KK/UZ pages.
...
### Internal Optimization (v2.0 - 2026-02-20)
...
### Anti-Spam & Observability (v1.5 - 2026-02-18)
...
### i18n / Locale Formatting (v1.5 - 2026-02-18)
...
### Fixed (v1.0 - Deployment & Performance)
