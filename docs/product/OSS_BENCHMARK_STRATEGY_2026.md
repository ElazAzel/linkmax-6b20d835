...
- ⏳ Следующие шаги: (a) subscription/usage-биллинг поверх Paddle/Lago, (b) DocuSeal embed для реального PDF-подписания, (c) миграция существующих трекеров кликов на `trackCanonicalEvent({ event: 'link_click', sourceObject: { type: 'link', id } })`, (d) в `smart-links.ts` / `offers.ts` использовать `Database['public']['Tables']` вместо `as never` после регенерации типов.
