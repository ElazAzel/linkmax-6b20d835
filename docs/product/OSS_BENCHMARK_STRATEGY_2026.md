...
- ✅ **Типизация сервисов**: `smart-links.ts` / `offers.ts` / `document-signatures.ts` переведены на сгенерированные `Database['public']['Tables'][…]['Insert' | 'Update']` из `@/integrations/supabase/types`. Убраны все `as never` касты — теперь insert/update валидируются типами PostgREST.
- ✅ **P2 Subscription & Usage Billing (Lago-модель)**: таблицы `offer_subscriptions` (recurring: interval, current period, cancel_at_period_end, provider-agnostic) и `usage_events` (metric_code, quantity, unit_amount_cents, occurred_at, unique per seller external_id — идемпотентность), RPC `aggregate_usage(subscription, metric, period_start, period_end)` и сервис `src/lib/money/subscriptions.ts` (`createOfferSubscription`, `advanceBillingPeriod`, `cancelOfferSubscription`, `recordUsageEvent`, `aggregateUsage`, `computeCurrentPeriodCharge`). RLS: продавец управляет своими подписками/событиями, клиент видит собственные. Не привязано к платёжному провайдеру — Robokassa/Paddle/Stripe подключаются на уровне adapter → checkout.
- ✅ **P2 DocuSeal Embed (Trust OS)**: edge-функции `docuseal-create-submission` (создаёт submission по `template_id`, авторизует вызывающего через `is_zone_member`, сохраняет `submission_id/slug/embed_src` в `document_signatures.metadata.docuseal`) и `docuseal-webhook` (HMAC-SHA256 верификация `DOCUSEAL_WEBHOOK_SECRET`, маппинг `form.viewed/started/completed/declined` → статусы `viewed/signed/declined`, запись `signed_at`, `signature_data.audit_log_url`, `ip`, `ua`). Клиентский адаптер `src/lib/trust/docuseal.ts` (`createDocusealSubmission`, `getDocusealEmbedSrc`) — фронт рендерит iframe / `@docuseal/react` по возвращённому `embed_src`. Провайдер-агностичный контракт: Documenso подключается тем же паттерном.
- ⏳ Следующие шаги: (a) subscription/usage-биллинг поверх Paddle/Lago (реальные recurring-платежи), (b) шаблоны договоров/актов + автосоздание DocuSeal submissions из pipeline stages.
